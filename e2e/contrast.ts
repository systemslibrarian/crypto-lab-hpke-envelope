 ...
 *
 *  - text over a background *gradient* or a translucent wash — axe declines to
 *    compute a ratio and files the node under `incomplete`. Every state surface
 *    on this page is a `color-mix(..., transparent)`: `.chip-ok` and
 *    `.chip-alarm` tint 14% of a fixed hue over the panel, `mark.byte-changed`
 *    tints 26% of `--mark-line` behind the bytes that moved, the hero's "why it
 *    matters" card is 6% of `--accent`, and `<select>` paints a data-URI chevron
 *    over its fill. None of those is a flat colour axe can read off one
 *    declaration.
 ...
 *    `opacity: .85`, so the subtitle under the H1 is not the ink the stylesheet
 *    names for it.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The nonce table
 *    lives in a `.tablewrap` (`overflow-x: auto`) and prints four columns of
 *    12-byte hex, so at 380px most of it is clipped away. Content scrolled past
 *    the client box is not dimmed or partly drawn — it is absent from the frame,
 *    and asking what colour it sits on has no answer. Its rect is still to the
 *    right of every ancestor's box, so the ancestor walk would find nothing
 *    behind it and fall through to white, inventing a failure per cell. Skip it,
 *    and rely on the wider viewport where the very same cell is visible and
 *    measured for real.
 *
 ...
 *    has no inline SVG carrying text — the `<select>` chevron is a data-URI
 *    background image and the shared top bar's icons have no `<text>` — so the
 *    rule never fires here. It is kept because it is cheap and silent when
 *    unused: an `<svg>` draws a shape as a `<rect>` and then its label as a
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: this page mixes in BOTH
     * `srgb` (the chips, the changed-byte marks, the stage-active glow) and
     * `oklab` (the hero card), and Chromium reports every one of those to
     ...
     * fabricates failures (a chip's ink read as sitting on the bare panel
     * rather than on its own tint) and, far worse, could hide a real one. The
     * 2D canvas is the browser's own colour
     ...
     * for a gradient that spans its element and wrong for a small one anchored
     * to a corner, and judging a page-tall element by a corner stop invents
     * backdrops nothing sits on. So each gradient is *sampled* at the text's
     ...
     * non-gradient layer is unmeasurable and paints nothing — which is the right
     * answer for this page's one `background-image`, the `<select>` chevron,
     * a `url(data:image/svg+xml,...)` that covers 0.9rem at the right edge and
     * is nowhere near the option text.
     */
     ...
     *
     * The mode panel renders one element PER BYTE of `key_schedule_context` (65
     * of them, each a `<span>` or `<mark>`), the nonce table another per byte
     * per sealed message, and every stage's expanded internals adds a dozen
     * full-width hex blocks. All of them walk the same handful of panel
     * ancestors. Without the caches the pass re-reads the same computed styles
     ...
     * This page uses the recipe verbatim — `.sr-only` is
     * `position: absolute; width: 1px; height: 1px; clip: rect(0 0 0 0)`, and
     * the nonce table's `<caption>` is one. Without this guard the walk
     * composites that caption against whatever surface it happens to sit on and
     * INVENTS a failure for text nobody can see. Deliberately narrow — only a
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has four: the "For the cryptographer" panel
      // and one "Show the real intermediates" per pipeline stage. The gate opens
      // each by clicking its summary and scans the open state for real, rather
      // than force-setting `.open` from script as the old spec did.
     ...
     * page's `.chip-icon` glyphs (✓ ✗ ⚠ ·) and the `.pipe-arrow-glyph` arrows
     * are `aria-hidden` duplicates of text sitting immediately beside them.
     *
     ...
     * page was enumerated and measured by hand instead; both inherit their
     * chip's or arrow's own colour, so both are covered by the measurement of
     * the text they sit next to.
     */
     ...
     * `html { background-color: #0b1120 }` (and the light override on
     * `html[data-theme='light']`) and runs many viewports tall once the stage
     * disclosures are open, so the canvas is load-bearing here. A lab that sets
